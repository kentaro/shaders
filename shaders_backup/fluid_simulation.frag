...
float fbm(vec2 p,float time,int octaves,float lacunarity,float gain){
    ...
    for(int i=0;i<7;i++){
        ...
        float noiseValue=noise(p*frequency+vec2(time*.1*float(i+1)));
...
vec2 curl(vec2 p,float t,float scale){
    ...
    float n1=fbm(p+vec2(eps,0.)+vec2(t*.3),t,FLUID_DETAIL,2.,.5);
    float n2=fbm(p-vec2(eps,0.)+vec2(t*.3),t,FLUID_DETAIL,2.,.5);
    float n3=fbm(p+vec2(0.,eps)+vec2(t*.3),t,FLUID_DETAIL,2.,.5);
    float n4=fbm(p-vec2(0.,eps)+vec2(t*.3),t,FLUID_DETAIL,2.,.5);
...
void main(){
    ...
    // Default intensity value to ensure visibility
    ...
    for(int i=0;i<FLUID_DETAIL;i++){
        ...
        pattern+=amplitude*fbm(rotated*frequency+vec2(time*.1),time,3,2.,.5);
